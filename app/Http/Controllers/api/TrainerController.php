    ...
    public function appVersionCheck(Request $request)
    {
       try{
        ...
        $version_count = DB::connection('mysql_2')->table('anudip_app_version')
                   ...
                   ->where('version', $request->app_version)
                   ->where('status',1)
                   ->count();
        if($version_count==0){
        ...
        try { 
            ...
            foreach($request->studentList as $member_id){
                ...
                if($incount==0){
                    ...
                    if($checkInTime>$time){
                        $checkInTime=DB::connection('mysql_2')->table('attendance_app')->where('member_id',$member_id)->where('atten_date',$request->attend_date)->where('punch_type','I')->update(['punch_time' =>$time,'bulk_type'=>1]);
