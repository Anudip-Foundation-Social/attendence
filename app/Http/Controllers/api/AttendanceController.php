                ...
                //return Response(['message' => 'inserted successfully vvv','status'=>1,'data11'=>$x->member_code],200);
                    ...
                    if($x['punch_out']!=null){
                        ...
                        if(sizeof($$details1)>0){
                            ...
                            Attendance::where('atten_date', $x['attend_date'])->where('user_id', $$details1[0]->user_id)->update(['punch_out'=>$x['punch_out'],'punch_out_lat'=>$x['lat'],'punch_out_long'=>$x['long'],'status'=>0,'punch_out_place'=>$x['location']]);
